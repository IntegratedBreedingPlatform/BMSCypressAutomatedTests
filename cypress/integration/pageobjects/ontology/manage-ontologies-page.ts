    ...
    checkTermIsInList(term: OntologyTerm) {
        this.getOntologyBrowserIframeBody().then(($iframe) => {
            ...
            cy.wrap($iframe).find(`om-list[om-list-name=${term.tabName.toLocaleLowerCase()}] > table > tbody > tr:not(.om-no-items)`)
                ...
                .then(($tr) => {
                    cy.wrap($tr).find('td:nth-child(2) > span').should('exist').should('contain', <string>this.variableName);
                    cy.wrap($tr).find('td:nth-child(3) > span').should('exist').should('contain', <string>this.propertyName);
                    cy.wrap($tr).find('td:nth-child(4) > span').should('exist').should('contain', <string>this.methodName);
                    cy.wrap($tr).find('td:nth-child(5) > span').should('exist').should('contain', <string>this.scaleName);
                });
        });
    }
